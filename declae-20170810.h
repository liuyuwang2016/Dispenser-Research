...
float M_Cubic[16] = {					//Machine Coordinate rotation
										//-0.9991195821842409, 0.01024500417329827, -0.04068292498674497,0,
										//-0.001538653530792244, 0.9601237543232082, 0.2795711160503168,0,
										//0.04192484992579217, 0.2793875735851483, -0.9592626807527243,0,
	-0.9988447970206874, 0.0258234890973138, -0.04052429981557931, 0,
	0.01060584669668246, 0.9410046461788812, 0.3382273967105636, 0,
	0.04686776590097417, 0.337406880902864, -0.9401914747746079, 0,
	0, 0, 0, 1 };
float M_Cubic_inv[16] = {				//Machine Coordinate rotation inversion
										//-0.9991195821842408, -0.001538653530792337, 0.0419248499257922,0,
										//0.01024500417329835, 0.9601237543232084, 0.2793875735851484,0,
										//-0.04068292498674492, 0.2795711160503169, -0.9592626807527246,0,
	-0.9988447970206871, 0.01060584669668234, 0.04686776590097418, 0,
	0.0258234890973139, 0.9410046461788811, 0.3374068809028639, 0,
	-0.04052429981557925, 0.3382273967105637, -0.9401914747746077, 0,
	0, 0, 0, 1 };
...
void onMouseROI(int event, int x, int y, int flags, void* param);   //Mouse callback Func
																	//DataType: 0 = unsigned char, 1 = float
...
CameraSpacePoint* ROICameraSP_MechCoord_Storage = nullptr;			//Probe tip on Machine Coordinate
CameraSpacePoint* ROICameraSP_MechCoord_Proj_Storage = nullptr;		//Probe tip projection on Machine Coordinate
...
void MtMove(void)
{
	...
	for (int i = 0; i < ROIStorageCount; i++)
	{
		...
		{
		case 0:
			sprintf(mybuffx, "%f", ROICameraSP_MechCoord_Storage[i].X * 1000/*+8.5*/);
			sprintf(mybuffy, "%f", ROICameraSP_MechCoord_Storage[i].Y * 1000);
			sprintf(mybuffz, "%f", ROICameraSP_MechCoord_Storage[i].Z * 1000 - ROICameraSP_MechCoord_Storage[i].Y * 1000 * tan(dev_theta));
			break;
		case 1:
			sprintf(mybuffx, "%f", ROICameraSP_MechCoord_Proj_Storage[i].X * 1000);
			sprintf(mybuffy, "%f", ROICameraSP_MechCoord_Proj_Storage[i].Y * 1000);
			sprintf(mybuffz, "%f", ROICameraSP_MechCoord_Proj_Storage[i].Z * 1000 - ROICameraSP_MechCoord_Proj_Storage[i].Y * 1000 * tan(dev_theta));
			break;
		}
...
void MtCheck(void)
{
	for (mtmove_step = 0; mtmove_step < ROIStorageCount; mtmove_step++)
	{
		...
		{
		case 0:
			value = ROICameraSP_MechCoord_Storage[mtmove_step].Z * 1000 - ROICameraSP_MechCoord_Storage[mtmove_step].Y * 1000 * tan(dev_theta);
			break;
		case 1:
			value = ROICameraSP_MechCoord_Proj_Storage[mtmove_step].Z * 1000 - ROICameraSP_MechCoord_Proj_Storage[mtmove_step].Y * 1000 * tan(dev_theta);
			break;
		}
		do
		{
...
void GLInit()
{
	...
	CubicPosi.y = 0;
	CubicPosi.z = 0;
	//Intersect.X = 0.4187436608598039;
	//Intersect.Y = 0.1204271721324741;
	//t的值
	Intersect.X = 0.2956173651736146;
	Intersect.Y = -0.1318814485147561;
	Intersect.Z = -1;
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case 'a':
		...
		//for (int i = 0; i < 5; i++)
		//{
		//	MtCmd("mt_out 12,1"); 12 glue out, 9 shining light
		//	Sleep(100);
		//	MtCmd("mt_out 12,0");
		//	Sleep(100);
		//}
...
void SceneWithBackground()
{
	...
	farDist = 20000;
	//double fx = 1064.78*0.001, fy = 1070.5*0.001, cx = 981.258*0.001, cy = 568.853*0.001;
	//glFrustum(-nearDist*cx / fx, nearDist*cx / fx, -nearDist*cy / fy, nearDist*cy / fy, nearDist, farDist);
...
void DrawMeshes()
{
	...
	{
		glPushMatrix();
		/*---------------------GL_TRIANGLE_STRIP 一种绘制三角形的方式
		http://blog.csdn.net/xiajun07061225/article/details/7455283 */
		glBegin(GL_TRIANGLE_STRIP/*GL_TRIANGLES*/);
		for (int i = 0; i < colorPointCount; i += 3)//3
		{
			//if (pCSPoints[i].Z != -1 * numeric_limits<float>::infinity() && pCSPoints[i + iWidthColor].Z != -1 * numeric_limits<float>::infinity() && pCSPoints[i + 1].Z != -1 * numeric_limits<float>::infinity() && pCSPoints[i + iWidthColor + 1].Z != -1 * numeric_limits<float>::infinity())
			//{
			glColor3ub(pBufferColor[4 * i], pBufferColor[4 * i + 1], pBufferColor[4 * i + 2]);
...
int glPrintf(const char *format, ...)
{
	...
	{
		/*sscanf_s()用这个函数从字符串中取出整数、浮点数以及字符串等等，使用简单*/
		sscanf_s(buffer, ">>glut(%i,%i)", &x0, &y0);
		glTranslatef(-x0, -y0, 0);
		//glTranslatef(-x0, -y0, 0);
		/*glRasterPos2f(x,y) 其实就相当于是 glVertex2f(x,y) 的坐标,只是 glRasterPos2f(x,y)
		用于显示 [字体] 时设置字符的起始位置, 而 glVertex2f(x,y) 是绘制一个2D点.
		且 glRasterPos2f(x,y) 仍受当前模视矩阵的影响,即glTranslatef()可改变字符显示的位置,
		glRasterPos2f(x,y)显示字体时,其位置可随模视矩阵的改变而改变.*/
		glRasterPos2f(4, viewport[3] - nHeight);
		//glRasterPos2f(50.0f, 20.0f);
		//cout << endl << "4 " << viewport[3] - nHeight;
	}
	/*strcmp比较两个是否相等，相等就传出0*/
