...
{   /*Virtual object's translate and rotate function*/
	glPushMatrix();
	//gltranslatef(x,y,z)将你绘点坐标的原点在当前原点的基础上平移一个(x,y,z)向量。
	...
	/*Intersect是抓的第三轴中心点的平移量*/
	glTranslatef(Intersect.X, Intersect.Y, Intersect.Z);
	//glMultMatrixf(m)其中矩阵m是有16个元素的一维数组，其按列定义了4*4矩阵，在其中m乘在已有矩阵的右边
...
void ShowImage()
{
	...
	//dstImage.copyTo(ROI);
	// mColorImg.copyTo(ROI);
	// finish or not finish the ROI getting process 画框框 
...
void onMouseROI(int event, int x, int y, int flags, void* param)
{
	...
	if (flags == CV_EVENT_FLAG_LBUTTON)
	{
		...
		ROI_p2 = Point(x, y);
		rectangle(ROI, ROI_p1, ROI_p2, Scalar(255, 255, 255), 2);
		cv::imshow("Color Map", ROI);
	}
}
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < cols; j++)
		{
			...
			if (IsRed > 155)
			{
				...
				ROI_Image.at<Vec4b>(i, j)[1] = 255;
				ROI_Image.at<Vec4b>(i, j)[2] = 255;*/
				ROI_Image.at<Vec4b>(i, j)[0] = 255;
...
void CameraSpaceROI()
{
	...
	//发现ROIcount也是ROI里面的深度点的数目
	//cout << "这里是ROI里面的点的数目：" << ROIcount << endl;
	//跑一个 loop，看一下 depth count 有几个
	for (int i = 0; i < ROIcount; i++)
	{
		int index1 = ROIPixel[i].x + ROIPixel[i].y * iWidthColor;
		//C++标准函数库通过template numeric_limits提供数据的极值。取代C所采用的预处理常数。infinity只是check无限大
		if (pCSPoints[index1].Z != -1 * numeric_limits<float>::infinity())
		{
			ROIDepthCount++;
		}
	}
	//ROIDepthCount大概8到12个。光看远点的红点的话
	//cout << "看这里是抓取到的ROI的深度点的数目" << ROIDepthCount << endl;
...
void RenderScene()
{
	...
	if (ROIDepthCount != 0 && ARFunc_IS_ON)
	{
		...
		/*Move the probe tip to the 把探针尖端的点移动到虚拟物体上面，然后用论文中的方法判断有没有到物体的上方*/
				...
				for (int TriCount = 0; TriCount < group->numtriangles; TriCount++)
				{
					...
					if (Psi*180.0f / 3.14159265358979323846 > 90.0f)
					{
						...
						case 3:
							...
							/*物体上面有一个圆形，他的normal的方向和一般的是相反的，需要来判断一下是不是触碰到了这个圆*/
							CollideCount_Circle++;
							break;
						default:
							CollideCount++;
	...
	if (ROIDepthCount > 0 && ROICameraSP != nullptr && !ROI_IS_COLLIDE)
	{
		...
		//ROICameraSP_MechCoord给点胶机数据的倒数第二步
		...
		/*After projection coordinate 投影过后的*/
	...
	//机器移动的时候我们需要让虚拟方块移动，所以我们要计算机器移动的距离deviation
...
void ROICameraSPStorage()
{
	if (ROICameraSP_Storage == nullptr)
	{
		//我们把笔尖的坐标转移到了机器坐标，但是我们需要把笔尖相对于机器的坐标和笔尖相对于opengl的坐标都存储起来，因为我们需要把它画出来，也需要利用她指引机器的移动
	...
	if (!ROI_IS_COLLIDE)
	{
		if (ROIStorageCount + 1 < StorageSize)
		{
			...
			//ROICameraSP_MechCoord_Storage存储的点最后给了点胶机使用
...
void traverseModel()
{
	...
	while (group) {
		...
		for (unsigned int i = 0; i < group->numtriangles; i++) {
			...
			/*因为模型画的是mm，但是这个里面画的都是公尺，因此需要填写一个缩放的scale*/
